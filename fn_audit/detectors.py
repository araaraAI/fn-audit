...
def _f(detector, severity, title, evidence, impact, probe) -> dict:
    return {"detector": detector, "severity": severity, "title": title,
            "evidence": evidence, "agent_impact": impact, "suggested_probe": probe}
...
def label_text_gap(conn, t, miner: LabelMiner, min_rows=20, column_alignment=0.0) -> list[dict]:
    ...
    for r in labels:
        ...
        out.append(_f(
            ...
            f'ラベル「{lab}」({n}件) の表記が本文に無い（一致率 {cov:.0%}）',
            ...
            f'「{lab}」で検索したエージェントは {n} 件を取りこぼし、'
            f'「該当なし」と結論する（実際には存在する）',
            (f'代替表記の候補「{dominant["term"]}」'
...
def catchall_composition(conn, t, miner: LabelMiner) -> list[dict]:
    ...
    for lab, n in miner.by_label.items():
        ...
        out.append(_f(
            ...
            f'包括ラベル「{lab}」({n}件) の中身は雑多でない（上位3語で約{share:.0%}）',
            ...
            f'「{lab}」は無視されやすいラベルであるため、この中の '
            f'約{share_rows}件の実在イベントにエージェントは到達しない',
            f'まず「{big[0]["term"]}」を本文検索して、独立したラベルが要るか判断する'))
    return out
...
def temporal_coverage(conn, t, quiet_factor=3.0) -> list[dict]:
    ...
    out.append(_f(
        ...
        f'観測範囲 {first} 〜 {last}（{span}日、実データのある日 {len(present)}日）',
        ...
        f'{first} より前を問い合わせたエージェントは必ず0件を得る。'
    ...
    if longest >= 7:
        out.append(_f("FN-5 coverage-hole", "medium",
                      f'最長 {longest} 日の空白がある',
                      {"longest_gap_days": longest},
                      f'空白期間を問い合わせたエージェントは0件を「不在」と読む',
                      '空白が休日か収集停止かを、期間を指定して原データで確かめる'))
...
def run_all(conn, t, sample=15000, pairs=None) -> list[dict]:
    ...
    if t.text_col and t.label_col:
        ...
        findings.append(_f(
            ...
            f'ラベル列「{t.label_col}」と本文「{t.text_col}」の語彙一致率 {align:.0%}',
            {"label_col": t.label_col, "text_col": t.text_col,
             "alignment": round(align, 3)},
            'この値が低いほど、ラベル名をそのまま検索語にしたエージェントは空振りする',
            '一致率が低いのに分類列として提供するなら、同義語展開を必ず挟む'))
        if t.label_is_content:
            ...
            findings.append(_f(
                ...
                f'ラベル列「{t.label_col}」は経路/状態列と判定したため語彙監査を行わない',
                {"label_col": t.label_col},
                'この列の値（配信先・処理状態など）は本文に現れないのが正常で、'
                '不一致は欠陥ではない',
                '内容分類の列が別にあるなら --label-col で指定して再実行する'))
